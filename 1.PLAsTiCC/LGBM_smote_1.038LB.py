...
def process_flux(df):
    ...
    df_flux = pd.DataFrame({
        ...
        index=df.index)
    tung = pd.concat([df, df_flux], axis=1)
    
    #implement idea number 2 
    flux_groupby_pb =tung.groupby('passband')['flux'].mean().to_frame(name= 'pass_band_flux').reset_index()
    tung_merge = tung.merge(right=flux_groupby_pb, how='left', on='passband')
    tung_merge['passband_diff'] = tung_merge['passband'] - tung_merge['pass_band_flux']
    # del tung_merge['pass_band_flux']
    # del flux_groupby_pb
    gc.collect()
...
def featurize(df, df_meta, aggs, fcp, n_jobs=4):
    ...
    df = process_flux(df)
    print('-------->>>>>> process flux column %s' %(df.columns))
...
def process_meta(filename):
    ...
    meta_dict = dict()
    print('this is the meta df just read')
    print (meta_df['distmod'].head())
    
    
    #implement the first point in https://www.kaggle.com/c/PLAsTiCC-2018/discussion/74679
    meta_df['distmod_inverse'] = 1/meta_df['distmod']**2
    del meta_df['distmod']
...
def main(argc, argv):
    ...
    aggs = {
        ...
        'passband_diff' :['min', 'max', 'mean', 'median', 'std', 'skew'],
    ...
    print('this is the train set peek into ')
    print(train.head(10))
    ...
    #implement suggestion no 2 
    
    
    print('this is full_train column')
    print(full_train.columns)
    print('this is full_train peek')
    print(full_train.head(10))    
